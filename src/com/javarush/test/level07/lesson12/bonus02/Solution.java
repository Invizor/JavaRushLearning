...
public class Solution
{
    public static void main(String[] args) throws IOException
    {
        ...
        ArrayList<String> listAnyCase = new ArrayList<String>();
        for (int i = 0; i < list.size(); i++)
        {
            String s = list.get(i);
            if(s.length() % 2 == 0)listAnyCase.add(s+" "+s);
            if(s.length() % 2 != 0)listAnyCase.add(s+" "+s+" "+s);
        }

        for (int i = 0; i < listAnyCase.size(); i++)
        {
            System.out.println(listAnyCase.get(i));
        }
    }
}
